...
bool isSyntacticallyCorrect(string dance) {
    ...
    while (dance.find("/") != string::npos) {
        ...
        else if (prefix.length() == 1 && directions.find(prefix) != string::npos) continue;
        else if (prefix.length() == 2 && isdigit(prefix.at(0)) && directions.find(prefix.at(1)) != string::npos) continue;
        else if (prefix.length() == 3 && isdigit(prefix.at(0)) && isdigit(prefix.at(1)) && directions.find(prefix.at(2)) != string::npos) continue;
        else return false;
    }

    //once all beats (dashes) are gone, checks theres nothing left
    if (dance.length() != 0) return false;
...
int translateDance(string dance, string& instructions, int& badBeat) {
    ...
    if (!isSyntacticallyCorrect(dance)) return 1;
...
int main() {
    while (true) {
        ...
        else cout << "Error: " << translation << " BadBeat: " << badBeat << endl;
    }
}